...
function stripDirectives(doc, except) {
    ...
    function excepted(dir) { return Boolean(except.find(function (f) { return dir.name.value === f; })); }
    function stripObjectDirectives(node) {
        var fields = node.fields ? node.fields.map(stripFieldDirectives) : node.fields;
        return __assign({}, node, { fields: fields, directives: node.directives.filter(excepted) });
    }
    ...
    function stripFieldDirectives(node) {
        ...
        return __assign({}, node, { arguments: args, directives: node.directives.filter(excepted) });
    }
    function stripArgumentDirectives(node) {
        return __assign({}, node, { directives: node.directives.filter(excepted) });
    }
    ...
    function stripEnumDirectives(node) {
        ...
        return __assign({}, node, { values: values, directives: node.directives.filter(excepted) });
    }
